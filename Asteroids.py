...
def group_collide(ship, group):
    for other_object in set(group):
        if other_object.collide(ship):
...
def rock_spawner():
    ...
    if len(rock_group) < 12 :
        ...
        t = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        if dist(t.pos, my_ship.pos) >= (t.radius + my_ship.radius + 5):
            rock_group.add(t)
